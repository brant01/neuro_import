...
def load_file(filename):
    ...
    if data_present:
        ...
        position = fid.tell()
        print(f"File cursor position: {position}")
...
def read_one_data_block(data, header, index, fid):
    ...
    position = fid.tell()
    print(f"File cursor position: {position}, Index: {index}, Samples per block: {samples_per_block}")
...
def calculate_data_size(header, filename, fid):
    ...
    print(f"Num Blocks: {num_blocks}, Bytes remaining: {bytes_remaining}, Bytes per Block: {bytes_per_block}, Num Samples: {num_samples}")
...
def initialize_memory(header, num_samples):
    ...
    for key, value in data.items():
        print(f"{key} dimensions: {value.shape}")
...
def print_progress(i, target, print_step, percent_done):
    ...
    if fraction_done >= percent_done:
        ...
        print(f"Current: {i}, Total: {target}")
        percent_done += print_step
